...
void bst_insert(bst *t, item x){ //oui
    t->root = node_insert(t->root, x);
}

bst *bst_from_array(item arr[], int len){ //oui
...
bool bst_member(bst *t, item x){ //oui
...
int bst_height(bst *t){ //oui
    return node_height(t->root);
}

void node_write_to_array(node *n, item arr[], int *offset_ptr){ //oui
    if (n == NULL) return; 
...
item *bst_to_array(bst *t, int *nb_elts){ //oui
...
void bst_delete(bst *t, item x){ //oui
...
void bst_in_order_print(bst *t){ //oui
